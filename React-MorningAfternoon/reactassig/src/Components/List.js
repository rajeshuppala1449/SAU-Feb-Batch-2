...
class List extends Component {
    ...
    // 20210227191811
    // https://www.googleapis.com/books/v1/volumes?q=flowers+inauthor:keyes&key=AIzaSyAzabxC_SV061xaXUsdmYIAJDUPEAMckc4
    handleChange = (event) => {
        this.setState({ value: event.target.value })
        if (this.state.value == "") {
            this.setState({ renderlist: [] })
        } else {
            var newarr = this.state.list.filter((ele) => ele.title.includes(this.state.value))
            this.setState({ renderlist: newarr })
    ...
    render() {
        ...
        return (
            <div>
                <div>
                    <div style={{ height: "70px", boder: "2px black solid", backgroundColor: "#e0e0ad" }}>
                        <div style={{ display: "flex", paddingLeft: "27%", paddingTop: "20px" }}>

                            <input style={{ width: "600px", height: "25px" }} type="text" placeholder="search books" value={this.state.value} onChange={this.handleChange}></input>
                        </div>
                    </div>
                ...
                <div style={{ margin: 'auto', marginTop: "15px" }}>
                    <div style={{ display: 'flex', flexWrap: 'wrap', paddingLeft: "110px" }} >
                        {arr.map((item) => <Card item={item}></Card>)}
                    </div >
                </div>
            </div>
        )
